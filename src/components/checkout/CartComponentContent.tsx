...
const CartComponentContent = ({className}: { className: string; }) => {
    ...
    const {mutateAsync: removeProduct} = useMutation(provider.removeProductFromCart)
    ...
    const {data: availableCountries} = useQuery('cart-available-countries', provider.getAvailableCountries);
    const {data, isLoading, refetch} = useQuery('cart', provider.getProducts);
    const {data: countryData, refetch: refetchCountry} = useQuery('cart-country', provider.getCountry);
    ...
    const sumOfProducts = products?.reduce((prev, curr) => {
    ...
    const isSomeProductDeliverable = products?.some(product => product.details?.isDeliverable === true) ?? false;

    const selectedCountry = (countryData != null && !isErr(countryData) && countryData != '') ? countryData : undefined;
        ...
        <div className={`${className} ics-flex ics-flex-col`} ref={productsParent}>
            ...
            {(!isLoading && (products == null || products.length == 0)) && (
                <div className={'ics-flex ics-flex-col ics-gap-4 ics-p-4'}>
                    <p className={'dark:ics-text-neutral-400 ics-text-neutral-700'}>Brak produktów w koszyku.</p>
                </div>
            )}

            <div className={'ics-max-h-[70vh] ics-overflow-clip ics-overflow-y-scroll'}>
            ...
            {(sumOfProducts > 0 && currency != null) && (
                <>
                    <div className="ics-mx-3 ics-flex ics-flex-col">
                        <div className={'ics-flex ics-flex-row ics-justify-between ics-text-lg'}>
                            <p className={'dark:ics-text-neutral-200 ics-text-neutral-800 ics-text-xl'}>Suma:</p>
                            <div className={'ics-flex ics-flex-col ics-items-end'}>
                                <p className={'dark:ics-text-neutral-200 ics-text-neutral-800 ics-font-medium'}>
                                    {formatCurrency(sumOfProducts, currency)}
                                </p>
                                {isSomeProductDeliverable && (
                                    <p className={'ics-opacity-70 ics-text-right ics-text-base'}>+ dostawa</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    {(countries && countries?.length > 1) && (
                        <div>
                            <h4 className={'ics-ml-3 ics-text-sm dark:ics-text-neutral-400 ics-text-neutral-700'}>Wybierz kraj</h4>
                            <CountrySelector
                    ...
                    <div className={'ics-mb-3 ics-flex ics-flex-col'}>
                        ...
                        {errMsg.length > 0 && (
                            <p className={'ics-text-red-500 ics-text-sm ics-text-center'}>{errMsg}</p>
                        )}
                    </div>
                </>
            )}
