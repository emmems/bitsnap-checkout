...
export interface CartMethods {
    ...
    justRedirectToPayment: (args: { email?: string; productID: string; name?: string; country?: string; marketingAgreement?: boolean }) => Promise<Err | {
        url: string
    }>;
}
...
export const getCheckoutMethods: (projectID: string) => CartMethods = (projectID) => {
    ...
    return {
        ...
        async getAvailableCountries(): Promise<Err | { name: string; code: string }[]> {
        ...
        async getNumberOfElementsInCart(): Promise<number> {
            return getCheckout()?.products?.reduce((acc, product) => acc + product.quantity, 0) ?? 0;
        },

        async getProducts(): Promise<Err | {
            id: string;
            productID: string;
            quantity: number;
            ...
            const result = await fetch(buildURL(projectID, `/products?${params.toString()}`), {
                method: 'GET'
            });
            ...
            products.forEach(product => {
                // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                // @ts-expect-error
                product['details'] = payload.result?.find(el => el.id === product.productID);
            });
        ...
        async removeProductFromCart(args: { id: string }): Promise<Err | void> {
            ...
            const newCheckout = {
                ...checkout,
                products: checkout?.products?.filter(product => product.id !== args.id)
            }
        ...
        async updateQuantity(args: { id: string; quantity: number }): Promise<Err | void> {
            const checkout = getCheckout();
        ...
        async justRedirectToPayment(args: {
            ...
            if (args.marketingAgreement === true) {
                payload.additionalAgreements = [
                    { id: MARKETING_AGREEMENT_ID, name: '', required: true, answer: true}
                ]
            }
